...
核心文件是 [`AIDONE.md`](./AIDONE.md)：一份放在项目根目录的清单，让 AI coding agent 在说"完成了"之前，按它来交付。
...
```text
用户能看到的变化：可以从 Settings 页面把自己的数据导出成一个 JSON 文件。
改了哪些文件：SettingsView.tsx、db/export.ts、copy.ts。
AIDONE 检查：
- 安全（P0）：通过。没把密码、密钥写进代码；用户输错东西也不会让 App 崩。
- 产品质量（P1）：缺一项——"清除数据"是一删就没的操作，还没加二次确认。
- 风险（P2）：不适用（这只是本地导出，不涉及生产环境）。
跑了哪些检查：项目可以正常打包。代码风格检查在 2 个旧文件上报错，与本次改动无关。
你自己怎么验：打开 Settings -> Export Data，看 JSON 是不是真的下载下来。
还剩什么风险："清除数据"是一删就没的操作，给真用户用之前应该再加道防线。
```

重点不是 AI 话多，是让 AI 像工程师向不懂编程的老板汇报一样，讲清楚改了什么、验过没有、还剩什么风险。
